...
import { ChevronLeft, ChevronRight, Database, Video } from "lucide-react";
...
type ApplicationPreviewViewerProps = {
...
export function ApplicationPreviewViewer({
  filePreview,
  isLoading,
  selectedEntry,
}: ApplicationPreviewViewerProps) {
  if (isLoading) {
  ...
  if (isVideoPreview(filePreview)) {
    return <VideoFileViewer filePreview={filePreview} />;
  }
  ...
  return <MetadataViewer filePreview={filePreview} />;
}

function isVideoPreview(filePreview: FileFormatPreview) {
  if (filePreview.kind === "mp4" || filePreview.kind === "m4v") {
    return true;
  }

  const extension = filePreview.mediaPath?.split(".").pop()?.toLowerCase();

  return extension === "mp4" || extension === "m4v";
}

function MetadataViewer({ filePreview }: { filePreview: FileFormatPreview }) {
  return (
...
function ImageFileViewer({ filePreview }: { filePreview: FileFormatPreview }) {
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_13rem] text-[11px]">
      ...
      <MetadataSidebar filePreview={filePreview} />
    </div>
  );
}

function VideoFileViewer({ filePreview }: { filePreview: FileFormatPreview }) {
  const videoSource = filePreview.mediaPath
    ? convertFileSrc(filePreview.mediaPath)
    : "";

  return (
    <div className="grid h-full min-h-0 grid-cols-[minmax(0,1fr)_13rem] text-[11px]">
      <div className="grid min-h-0 place-items-center overflow-hidden bg-black p-1.5">
        {videoSource ? (
          <video
            src={videoSource}
      ...
      <MetadataSidebar filePreview={filePreview} icon="video" />
    </div>
  );
}
...
function MetadataSidebar({
  ...
  filePreview: FileFormatPreview;
  icon?: "database" | "video";
}) {
  const Icon = icon === "video" ? Video : Database;
